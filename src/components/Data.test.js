...
test("Render Book details and checks borrow button click", () => {

  const handleBorrow = jest.fn();

  const { getByText, getByRole } = render(<Book book={bookData} handleBorrow={handleBorrow} handleReturn={()=>{}} />);

  expect(getByText("Book-1")).toBeInTheDocument();
  expect(getByText("Ram")).toBeInTheDocument()

  fireEvent.click(getByRole("button", { name: /borrow/i }));

  expect(handleBorrow).toHaveBeenCalledTimes(1);
  expect(handleBorrow).toHaveBeenCalledWith(bookData.id);
});